           ...
           (if (closure? cl)
               (let* ([f (closure-fun cl)]
                 ...
                 (eval-under-env (fun-body f)
                                 (append (if fn
                                             (list (cons fn cl) (cons (fun-formal f) v1))
                                             (list (cons (fun-formal f) v1)))
                                         (closure-env cl))))
...
(define mupl-map (fun #f "f" (fun "hf" "xs" (ifaunit (var "xs")
                                                     (aunit)
                                                     (apair (call (var "f") (fst (var "xs")))
                                                            (call (var "hf") (snd (var "xs"))))))))

(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "i" (fun #f "xs" (call (call (var "map")
