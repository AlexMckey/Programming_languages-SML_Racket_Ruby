...
# hw7testsprovided.rb
require 'c:/Ruby/lib/ruby/2.0.0/x64-mingw32/coverage.so'
Coverage.start

require "d:/dropbox/devs/projects/ruby/hw7.rb"
...
#Shift furhter tests
pp = Shift.new(THREE,FIVE,Shift.new(THREE,FIVE,Point.new(THREE,FIVE)))
pp1 = pp.preprocess_prog.eval_prog([])
if not (pp1.x == 9 and pp1.y == 15)
    puts "Point Shift Shift should shift e by dx and dy"
end
...
pp2 = LineSegment.new(3.2,4.1,3.2,4.1)
pp3 = pp2.preprocess_prog
if not (pp3.is_a? Point and pp3.x == 3.2 and pp3.y == 4.1)
    puts "preprocess_prog adv not properly turning linesegment to a point"
end

#advanced intersect tests
xdls = LineSegment.new(ONE,TWO,THREE,6.0)
xdl  = Line.new(ONE,TWO)
xdp  = Point.new(2.0,4.0)
xdvl = VerticalLine.new(2.0)

xd1 = xdls.intersect(xdp)
xd2 = xd1.intersect(xdl)
xd3 = xd2.intersect(xdvl)
if not ((xd3.is_a? Point))
    puts "LineSegment intersect advanced not working properly"
    else
    puts "Line segment with Point OK"
    puts xd3.x
    puts xd3.y
end

xd4 = xdls.intersect(xdp)
xd5 = xdl.intersect(xdvl)
xd6 = xd4.intersect(xd5)
if not ((xd6.is_a? Point))
    puts "LineSegment intersect advanced not working properly"
    else
    puts "Line segment with Point OK"
    puts xd6.x
...
xd7 = xdvl.intersect(xdp)
xd8 = xdl.intersect(xdls)
xd9 = xd7.intersect(xd8)
if not ((xd9.is_a? Point))
    puts "LineSegment intersect advanced not working properly"
    else
    puts "Line segment with Point OK"
    puts xd9.x
...
xdp1 = Point.new(2.5,5.0)
xd10 = xdvl.intersect(xdp1)
xd11 = xdl.intersect(xdls)
xd12 = xd11.intersect(xd10)
if not ((xd11.is_a? Point) and (xd12.is_a? NoPoints))
    puts "LineSegment intersect advanced not working properly"
    else
    puts "Line segment with Point OK!"
    puts xd11.x
...
rv = Coverage.result["d:/dropbox/devs/projects/ruby/hw7.rb"]
rv.each_with_index {|el,i| puts i+1 if el==0}
puts "total: " + rv.count{|x| x==0}.to_s